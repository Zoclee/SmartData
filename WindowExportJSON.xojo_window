...
#tag Events cmdExport
	#tag Event
		Sub Action()
		  ...
		  jsonType.Name = "JSON Files"
		  jsonType.MacType = "JSON"
		  jsonType.MacCreator = "json"
		  ...
		  if columns.Ubound >= 0 then
		    ...
		    if f <> nil then
		      ...
		      if mData <> nil then
		        item = mData.GetJSON(columns)
		      else
		        item = WindowMain.Connector.GetJSON(WindowMain.TableName, columns, WindowMain.TableFilter)
		      end if
		      
		      tos.Write item.ToString()
